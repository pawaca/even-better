...
> Describes Claude Code CLI **2.1.201** and Codex CLI **0.142.5**. External refs
...
**How located**: `findSessionFile` (`src/transcript.ts:14-22`) does **not** know
...
**Records consumed** (`src/transcript.ts:24-107`):
...
- **`tool_result.is_error` is read.** The canonical error block is
  ...
  see §2.)
- **`message.model` is read.** Every assistant record carries it (observed
...
- **`AskUserQuestion` `tool_use.input.questions[]` builds the question.**
...
**How located**: `findCodexSessionFile` (`src/codex-transcript.ts:20-44`) does a
...
**Codex dedup**: the same text arrives twice (`event_msg.{user,agent}_message`
...
**Contract gaps:**
- **`model` is read.** Codex persists it in the `turn_context` record as a
  **required** field: `pub model: String` in
  ...
  (`src/codex-transcript.ts`) reads it for `/api/info`.
...
|---|---|---|---|
| `extractModel` (`/api/info`) | model name (status-bar regex, claude-only → codex "Unknown") | claude `message.model`; codex `turn_context.payload.model` | **wired** — scrape is fallback-only |
...
**Already structured:** codex tool completion via
...
`tool_result.is_error`; AskUserQuestion options from
